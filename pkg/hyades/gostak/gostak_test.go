...
func TestArith(t *testing.T) {
	...
	fg.Eval(progn)
	if fg.Len() != 1 {
		t.Errorf("Bad stack len %v", fg.Len())
	}
	if fg.At(0).(float64) != float64(3.0) {
		t.Errorf("Bad top value %v", fg.At(0))
	}
}
...
func TestQuotation(t *testing.T) {
	...
	fg.Eval(progn1)
	if fg.At(0).(float64) != float64(10.0) {
		t.Errorf("Bad top value %v", fg.At(0))
	}
	...
	fg.Eval(progn2)
	if fg.At(0).(float64) != float64(20.0) {
		t.Errorf("Bad top value %v", fg.At(0))
	}
